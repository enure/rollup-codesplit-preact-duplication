...
function getNodeProps$1(vnode) {
	var props = extend$1({}, vnode.attributes);
	props.children = vnode.children;
...
function diff$1(dom, vnode, context, mountAll, parent, componentRoot) {
	...
	var ret = idiff$1(dom, vnode, context, mountAll, componentRoot);
...
var recyclerComponents$1 = [];

function createComponent$1(Ctor, props, context) {
	var inst,
	    i = recyclerComponents$1.length;
...
function setComponentProps$1(component, props, renderMode, context, mountAll) {
	...
	if (renderMode !== 0) {
		if (renderMode === 1 || options$1.syncComponentUpdates !== false || !component.base) {
			renderComponent$1(component, 1, mountAll);
		} else {
...
function renderComponent$1(component, renderMode, mountAll, isChild) {
	...
	if (!skip) {
		...
		if (typeof childComponent === 'function') {
			...
			var childProps = getNodeProps$1(rendered);
			inst = initialChildComponent;

			if (inst && inst.constructor === childComponent && childProps.key == inst.__key) {
				setComponentProps$1(inst, childProps, 1, context, false);
			} else {
				toUnmount = inst;
				...
				inst.nextBase = inst.nextBase || nextBase;
				inst._parentComponent = component;
				setComponentProps$1(inst, childProps, 0, context, false);
				renderComponent$1(inst, 1, mountAll, true);
			}
...
function buildComponentFromVNode$1(dom, vnode, context, mountAll) {
	...
	if (c && isOwner && (!mountAll || c._component)) {
		setComponentProps$1(c, props, 3, context, mountAll);
		dom = c.base;
	} else {
		if (originalComponent && !isDirectOwner) {
		...
		setComponentProps$1(c, props, 1, context, mountAll);
		dom = c.base;
...
extend$1(Component$1.prototype, {
	setState: function setState(state, callback) {
		if (!this.prevState) this.prevState = this.state;
		this.state = extend$1(extend$1({}, this.state), typeof state === 'function' ? state(this.state, this.props) : state);
		if (callback) this._renderCallbacks.push(callback);
		enqueueRender$1(this);
	},
	forceUpdate: function forceUpdate(callback) {
		if (callback) this._renderCallbacks.push(callback);
		renderComponent$1(this, 2);
	},
	render: function render() {}
});

var Shared = (function (Component) {
  function Shared () {
    Component.apply(this, arguments);
  }

  if ( Component ) Shared.__proto__ = Component;
  Shared.prototype = Object.create( Component && Component.prototype );
  Shared.prototype.constructor = Shared;

  Shared.prototype.render = function render () {
...
var Foo = (function (Component$$1) {
  function Foo () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) Foo.__proto__ = Component$$1;
  Foo.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Foo.prototype.constructor = Foo;

  Foo.prototype.render = function render$$1 () {
    return h( 'div', null, h( Shared, null ) );
  };
